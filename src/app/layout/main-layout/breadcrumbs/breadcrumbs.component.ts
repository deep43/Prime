  ...
  constructor(private router: Router, private route: ActivatedRoute, private refreshService: RefreshService) {
    this.router.events
      ...
      .subscribe(() => {
        ...
        if (this.router.url === '/monitoring/charts') {
          this.renderChartRefreshRow = true;
        } else {
          this.renderChartRefreshRow = false;
        }

        if (this.router.url === '/clients/clientdetails') {
          this.renderReportRow = true;
        } else {
          this.renderReportRow = false;
        }

        if (this.router.url === '/clients/createreport') {
          this.renderSaveRow = true;
        } else {
          this.renderSaveRow = false;
        }

        if (this.router.url === '/aggregator/aggregatorone') {
          this.renderAggreagatorOneRow = true;
        } else {
          this.renderAggreagatorOneRow = false;
        }

        if (this.router.url === '/aggregator/aggregatortwo') {
          this.renderAggreagatorTwoRow = true;
        } else {
          this.renderAggreagatorTwoRow = false;
        }
      });
  }

  openReportWizard(){
    const reportUrl = window.location.origin + '/#/clients/createreport';
